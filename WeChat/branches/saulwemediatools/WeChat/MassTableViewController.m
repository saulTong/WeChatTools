...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    ...
    if (publicClass.isS9On) {
        return [publicClass totalSectionWithDictionary:publicClass.friendContactDic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString * str = [NSString stringWithFormat:@"第 %ld 组好友",(long)indexPath.section + 1];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (publicClass.isS9On) {
        [publicClass addShareUserInfo:indexPath.section];
    } else {
        NSString * obj = [NSString stringWithFormat:@"%ld",(long)indexPath.section];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"POST_SELECT_VIEW" object:obj];
    }
}
